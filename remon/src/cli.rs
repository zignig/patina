...
pub fn do_peek(address: u32, port: &mut Box<dyn SerialPort>) -> Result<(), anyhow::Error>{
    // load addr register
    ...
    // read out the data
    let cmd = [2];
    do_cmd(port, &cmd).context("sending GET")?;
    let mut data = [0; 4];
...
pub fn do_poke(address: u32, value: u32, port: &mut Box<dyn SerialPort>) -> Result<(), anyhow::Error> {
    // load addr register
    let mut cmd = [3, 0, 0, 0, 0];
    cmd[1..].copy_from_slice(&address.to_le_bytes());
    do_cmd(port, &cmd).context("loading A")?;
    ...
    do_cmd(port, &cmd).context("sending PUT")?;
    Ok(())
}
