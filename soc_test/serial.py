...
class AsyncSerialPeripheral(wiring.Component):
    ...
    def __init__(self, *, name, divisor, rx_fifo_depth=16, tx_fifo_depth=16):
        super().__init__({"csr_bus": In(csr.Signature(addr_width=8, data_width=32))})

        regs = csr.Builder(
            addr_width=self.csr_bus.addr_width, data_width=self.csr_bus.data_width, name=name
        )
        ...
        self.csr_bus.memory_map = regs.as_memory_map()

        self._bridge = csr.Bridge(self.csr_bus.memory_map)
